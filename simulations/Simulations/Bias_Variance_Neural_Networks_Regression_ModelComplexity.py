...
def samplePredForEstimator(n_neigh, X_train, y_train):
    sample_Model = MLPRegressor(hidden_layer_sizes=n_neigh).fit(X_train, y_train)
    return sample_Model.predict(X_test)

def get_bias_variance(sampleCount, noOfModels, estimator_name, n_neigh):
    bias_variance_result = pd.DataFrame(columns=['hidden_layer','no_of_models','algorithm','bias','variance', 'mse'])
    ...
    s_linear = pd.Series(data={'hidden_layer':n_neigh, 'no_of_models':noOfModels, 'algorithm':'KNN',
...
def bias_variance_mse_graphics(noOfSamples, noOfModels, estimator_name, noOfHiddenLayers) :
    lenNoOfHiddenLayer = len(noOfHiddenLayers)
    bias_variance_result = pd.DataFrame()
    print('Building Model for samples ', noOfSamples)
    for i in range(0, lenNoOfHiddenLayer):
        noOfHiddenLayer = noOfHiddenLayers[i]
        print('Building models with n hidden layers : ', noOfHiddenLayer)
        bias_variance_result = bias_variance_result.append(get_bias_variance(40000,noOfModels, estimator_name, noOfHiddenLayer))
    bias_variance_result.reset_index(inplace=True)
    bias_variance_result.drop(['index'], axis=1, inplace=True)
    ...
    ax[0].plot(bias_variance_result.hidden_layer, bias_variance_result.bias)
    ax[0].plot(bias_variance_result.hidden_layer, bias_variance_result.variance)
    ax[0].legend(['bias','variance'])
    ax[0].set_xlabel('k')
        
    ax[1].plot(bias_variance_result.hidden_layer, bias_variance_result.mse)
    ax[1].legend(['mse'])
    ax[1].set_xlabel('k')
    plt.show()

noOfHiddenLayer = [10,25,50,75,100,200,400,800,1000,1500,2000,2500]
bias_variance_mse_graphics(40000, 100, 'MLP', noOfHiddenLayer)