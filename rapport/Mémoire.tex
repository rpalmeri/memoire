...
En utilisant la preuve intermédiaire (voir section \ref{intermediate_proof_b_subsection}), on obtient l'équation suivante :
...
\begin{equation}
	...
	\sigma^2 + E_x[(f(x) -\bar{g}(x))^2] + E_{x,D} [(\bar{g}(x) - g^{(D)}(x))^2] + 0
\end{equation}
